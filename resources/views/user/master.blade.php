...
<head>
    ...
    <style>
        ...
        .carousel-inner img {
            max-height: 400px;
            /* Change this value to set the desired maximum height */
            object-fit: contain;
            /* This ensures the image retains its aspect ratio */
        }
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0 ms-lg-4">
                    <li class="nav-item"><a class="nav-link {{ request()->routeIs('*dashboard*') ? 'active' : '' }}"
                            aria-current="page" href="{{ route('dashboard') }}">Home</a></li>
                    @if (Auth::user())
                        <li class="nav-item"><a class="nav-link {{ request()->routeIs('*history*') ? 'active' : '' }}"
                                aria-current="page" href="{{ route('history.book') }}">History</a></li>
                    @endif
                ...
                @if (Auth::user())
                    <div class="d-flex gap-2">
                        <a href="{{ route('guest.form.booking') }}" class="btn btn-outline-secondary"
                            role="button">Book
                            Now</a>
